...
import { apiGetTransactionsByPeriod,
         apiCreateUpdate,
         apiRemove, } from './api/api';
...
export default function App() {
    const [selectedYearMonth, setSelectedYearMonth] = React.useState(getYearMonth());
    ...
    function incDecYearMonth(incDec) {
        ...
        //Faz o ++ ou -- de acordo com o incDec
          // console.log(index); console.log(incDec);
          //eval(`index${incDec}${incDec}`);
        if (incDec === '+') { index++; } else { index--; }
          //console.log(index); 

        //Limita o index em 0 ou na quantidade de YEARS_MONTHS
        if (index < 0 || index === YEARS_MONTHS.length) { return; }
    ...
    const handleModalSave = async (formData) => {
        ...
        //console.log(formData.yearMonthDay);

        if (!formData.yearMonthDay) { showMsg("É necessario escolher uma data", true);  return; }
    ...
    return (
        <div className="container">
            <h4 className="center">App React.JS API Node.JS integrado MongoDB Atlas</h4>
